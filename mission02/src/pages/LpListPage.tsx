...
export default function LpListPage() {
  ...
  const { data, isLoading, isError, refetch, fetchNextPage, hasNextPage, isFetchingNextPage } = useLpList(sort)

  const allLps = data?.pages.flatMap((page) => page?.data?.data ?? []) ?? []

  // useThrottle로 스크롤 이벤트 제어 (1초에 한 번만 실행)
  const handleScroll = useCallback(() => {
    const scrollTop = window.scrollY
    const windowHeight = window.innerHeight
    const documentHeight = document.documentElement.scrollHeight

    console.log('[Throttle] 스크롤 이벤트 발생')
  ...
  return (
    <div>
      <div style={styles.sortBar}>
        <button onClick={() => setSort('oldest')} style={{ ...styles.sortBtn, ...(sort === 'oldest' ? styles.active : {}) }}>오래된순</button>
        <button onClick={() => setSort('latest')} style={{ ...styles.sortBtn, ...(sort === 'latest' ? styles.active : {}) }}>최신순</button>
      </div>
